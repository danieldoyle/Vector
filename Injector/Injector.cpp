...
using namespace ProcessInjector;

static unsigned int WM_GOKU = ::RegisterWindowMessage(L"Injector_GOKU");
static HHOOK _messageHookHandle;

//-----------------------------------------------------------------------------
//Spying Process functions follow
//-----------------------------------------------------------------------------
void Injector::Inject(System::IntPtr windowHandle, System::String^ assembly, System::String^ className, System::String^ methodName)
{
	...
	{
		Log("GetModuleHandleEx successful", true);
		DWORD processID = 0;
		DWORD threadID = ::GetWindowThreadProcessId((HWND)windowHandle.ToPointer(), &processID);

		if (processID)
		{
			Log("Got process id", true);
			HANDLE hProcess = ::OpenProcess(PROCESS_ALL_ACCESS, FALSE, processID);
			if (hProcess)
			{
				Log("Got process handle", true);
				...
				if (acmRemote)
				{
					Log("VirtualAllocEx successful", true);
					...
					{
						Log("SetWindowsHookEx successful", true);
						::SendMessage((HWND)windowHandle.ToPointer(), WM_GOKU, (WPARAM)acmRemote, 0);
...
{	            
	...
	System::String ^ pathname = applicationDataPath + "\\Injector.txt";
...
LRESULT __stdcall MessageHookProc(int nCode, WPARAM wparam, LPARAM lparam)
{
	if (nCode == HC_ACTION)
	{
		...
		if (msg != NULL && msg->message == WM_GOKU)
		{
			System::Diagnostics::Debug::WriteLine("Got WM_GOKU message");
